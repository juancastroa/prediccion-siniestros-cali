...
def cargar_registros():
    """Une el historico base con cualquier archivo nuevo en data/nuevos/."""
    partes = []

    archivo_base = HISTORICO_DIR / "dataset_siniestros_cali_limpio.csv"
    if archivo_base.exists():
        partes.append(pd.read_csv(archivo_base, low_memory=False))
    ...
    if not partes:
        raise SystemExit(
            "No se encontro ningun dato en data/historico/ ni data/nuevos/."
        )
...
def construir_serie_diaria(df):
    ...
    # Rellena huecos de dias sin ningun registro con 0
    rango_completo = pd.date_range(diario.index.min(), diario.index.max(), freq="D")
...
def generar():
    df = cargar_registros()
    serie = construir_serie_diaria(df)
