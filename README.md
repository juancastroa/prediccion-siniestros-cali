...
```
data/historico/serie_diaria_historica.csv   conteo por día, 2016 en adelante (liviano, ~50 KB)
data/nuevos/        aquí se sube el archivo del día, detalle por siniestro (CSV o Excel)
...
**Por qué la serie histórica está agregada por día:** el dataset original
(un registro por siniestro) pesa más de 25 MB, que es el límite de subida
por la web de GitHub. El modelo solo necesita el total por día, así que
`serie_diaria_historica.csv` guarda eso — apenas unos KB — y es lo único
que se sube una vez al crear el repositorio. Los archivos que subas día a
día en `data/nuevos/` sí pueden venir con el detalle registro por registro,
igual que el reporte original; el script los cuenta y sólo así se van
agregando a la serie diaria automáticamente.
